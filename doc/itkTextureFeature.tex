...
ITK already contains tools for texture analysis such as \doxygen{ScalarImageToTextureFeaturesFilter} or \doxygen{ScalarImageToRunLengthFeaturesFilter}. Each of these filters can be used in order to obtain a N-Dimensional texture map from a image.
...
\doxygen{itkScalarImageToTextureFeaturesImageFilter} computes the following features from the co-occurrence matrix:
...
\doxygen{itkScalarImageToRunLengthFeaturesImageFilter} computes the following features:
...
im = itk.read(sys.argv[1])
mask = itk.read(sys.argv[2])
...
result = itk.Image.VF83.New()
result = filtr.GetOutput()

itk.write(result, "result.nrrd")
\end{minted}
\normalsize
\newpage
\section{Results}
